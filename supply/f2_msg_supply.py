...
import f1_group_supply as groupSupply
import f2_msg_rabbit as msgRabbit
...
def save_to_sqlite(data, batch_code):
    ...
    # # 创建表（如果不存在）
    # c.execute('''
    #     CREATE TABLE IF NOT EXISTS valid_msg_data (
    ...
    for url, content in data.items():
        ...
        if content:
            # 检查 URL 是否已存在
            c.execute('SELECT COUNT(1) FROM valid_msg_data WHERE url = ?', (url,))
            exists = c.fetchone()[0] > 0

            if exists:
                # 更新现有记录的 updated_at
                c.execute('''
                    UPDATE valid_msg_data
                    SET
                        page_title = ?,
                        page_extra = ?,
                        url_type = ?,
...
def update_telegram_data():
    ...
    # 从数据库中获取原始数据
    c.execute('SELECT rowid, page_extra FROM valid_msg_data')
    rows = c.fetchall()
    ...
    for row in rows:
        ...
        if page_extra:
            ...
            # 更新数据库
            c.execute('UPDATE valid_msg_data SET members_subscribers = ?,url_type=? WHERE rowid = ?',
...
def getWebInfo(urlsAll, batchNo):
    ...
    c.execute('''
               ...
               ''')
    # 从数据库中删除旧数据
    c.execute('delete FROM valid_msg_data')
    conn.commit()
    conn.close()
...
def query_telegram_data():
    ...
    c.execute('''
        ...
        FROM valid_msg_data 
        WHERE length(url)-length(replace(url,'/',''))=4 and action_button in('View In Group','View In Channel')
    ''')
...
def query_delete_data():
    ...
    c.execute('''
        SELECT url
        FROM valid_msg_data 
        WHERE page_action in('View Post','Send Message','Open App')
    ''')
    data_link = c.fetchall()
...
def updateMsg():
    # url = "http://localhost:8080/cms/test/supplyMsgInfo"
    url = "http://a5b684db7f9d746b18f60bbe60781a74-8b898802e7cba134.elb.ap-southeast-1.amazonaws.com:8800/cms/content/supplyMsgInfo"
    headers = {
...
def api_import():
    ...
    print(f"msg api_import end,count:{count}")
    return count

def run():
    while True:
        msgCnt = api_import()
        groupCnt = groupSupply.api_import()
        if msgCnt == 0 and groupCnt == 0:
            print("no data，sleep 30 minutes")
            time.sleep(1800)
            continue
        API_ID = '26534384'
        ...
        with client:
            client.send_message(5380352615, f"{datetime.now()},supply end,msg:{msgCnt},group:{groupCnt}")

if __name__ == "__main__":
    csv_import()
    # groupSupply.csv_import()
    # run()
