...
def businessCode():
    ...
    while i < 20:
        ...
        if cnt == 0:
            ...
            groupCnt = cnt + groupCnt
    print(f"{datetime.now()},supply end,msg count:{msgCnt}, group count:{groupCnt}, emptyLink count:{emptyCnt}")
    if msgCnt < 100 and groupCnt < 100 and emptyCnt < 100:
        print(f"{datetime.now()},no data，sleep 10 minutes")
        time.sleep(600)
    else:
        response = requests.get('https://api.ipify.org', timeout=5)
        sendMsg(f"{datetime.now()},supply end\nip:{response.text},msg:{msgCnt},group:{groupCnt}")
